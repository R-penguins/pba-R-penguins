    ...
    for (unsigned int jx = 0; jx < num_div; ++jx) {
      for (unsigned int jy = 0; jy < num_div; ++jy) {
        if (abs_diff(ix, jx) <= 1 && abs_diff(iy, jy) <= 1) { // this grid is near to the particle as the grid indexes are close
          ...
          const unsigned int np = acc.grid2idx[jy * num_div + jx + 1] - acc.grid2idx[jy * num_div + jx];
          if (np == 0) { continue; }
          const auto& cg = acc.grid2cg[jy * num_div + jx];
          particles[ip].force += gravitational_force(cg - particles[ip].pos) * np;
        }
      }
    }
  }
