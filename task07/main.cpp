...
void solve_laplace_gauss_sidel_on_grid(
  ...
  for (unsigned int ix = 0; ix < grid_size; ++ix) {
    for (unsigned int iy = 0; iy < grid_size; ++iy) {
      ...
      const float neighbor_sum = vtx2val[idx_center - grid_size] + vtx2val[idx_center + grid_size] + vtx2val[idx_center - 1] + vtx2val[idx_center + 1];
      vtx2val[idx_center] = neighbor_sum / 4.0f;
    }
  }
}
